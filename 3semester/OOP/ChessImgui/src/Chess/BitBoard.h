...
inline uint64_t SquareCount(BitBoard board) { return __popcnt64(board); }
#else
inline Square GetSquare(BitBoard board) {
  board = board & (~board + 1);
...
inline uint64_t SquareCount(BitBoard board) {
  size_t count = 0;
  for (; board != 0; count++) board = board & (board - 1);
  return count;
}
#endif

inline BitBoard BitBoardFile(Square square) {
  return 0x0101010101010101ull << (square & 0b00000111);
}

inline BitBoard BitBoardRank(Square square) {
  return 0x00000000000000FFull << (square & 0b11111000);
}
...
inline void PrintBitBoard(BitBoard board) {
  ...
  if (BoardFormat::s_BoardFormat.Coordinates)
    std::cout << (BoardFormat::s_BoardFormat.Orientation == White
                      ? "  abcdefgh\n"
                      : "  hgfedcba\n");
