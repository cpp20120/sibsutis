...
void Application::RenderEnginePanel(bool* show) {
  ...
  if (s_SelectedEngine == m_Engines.end()) {
    ...
    if (ImGui::BeginPopupModal("Create engine", nullptr,
    ...
    for (auto it = m_Engines.begin(); it != m_Engines.end();) {
      ...
      if (ImGui::Button("Start engine")) {
        ...
        try {
          ...
          m_RunningEngine->SetUpdateCallback(
              [this](const Engine::BestContinuation& c) { OnEngineUpdate(c); });
    ...
    if (ImGui::Button("Stop engine")) {
      ...
      if (!m_BestContinuation.Mate) {
        ...
        const char* text =
            (m_Board.GetPlayerTurn() == White) ? "Score: M%i" : "Score: -M%i";
...
void Application::OnEngineUpdate(
    const Engine::BestContinuation& bestContinuation) {
