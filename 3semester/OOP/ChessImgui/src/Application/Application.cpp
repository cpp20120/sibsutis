...
Application::Application(uint32_t width, uint32_t height,
    ...
    : m_WindowProperties{width, height, name},
      m_ChessViewportSize(width, height) {
  ...
  glfwSetWindowCloseCallback(m_Window,
                             [](GLFWwindow* window) { Get().OnWindowClose(); });
...
void Application::Init() {
  Renderer::Init(glm::ortho(-8.0f, 8.0f, -4.5f, 4.5f),
                 Resources::Shaders::VERTEX_SHADER,
                 Resources::Shaders::FRAGMENT_SHADER);
  ...
  std::shared_ptr<Texture> chessPieces =
      std::make_shared<Texture>(Resources::Textures::CHESS_PIECES,
                                sizeof(Resources::Textures::CHESS_PIECES));

  const float tileSize = (float)chessPieces->GetWidth() / 6.0f;
  for (int y = 0; y < 2; y++)
    for (int x = 0; x < 6; x++)
      m_ChessPieceSprites[y * 6 + x] = std::make_shared<SubTexture>(
          chessPieces, glm::vec2(x, y), glm::vec2(tileSize));
...
void Application::RenderImGui() {
  static bool s_ShowSettingsWindow = true, s_ShowFENWindow = true,
  ...
  if (s_ShowFENWindow) {
    ...
    if (ImGui::InputText("##FEN", m_BoardFEN.data(), m_BoardFEN.size(),
    ...
    if (ImGui::Button("Reset board")) {
      m_Board.Reset();
      m_BoardFEN = Board::START_FEN;
      if (m_RunningEngine) m_RunningEngine->SetPosition(m_BoardFEN);
    }
...
void Application::OnWindowClose() { m_Running = false; }
...
void Application::OnKeyPressed(int32_t key, int32_t scancode, int32_t action,
                               int32_t mods) {
    ...
    if (action == GLFW_PRESS) {
      ...
      if (point.x > -4 && point.x < 4 && point.y > -4 && point.y < 4) {
        ...
        if (m_SelectedPiece != INVALID_SQUARE &&
            m_SelectedPiece != selectedSquare) {
          if (m_LegalMoves & (1ull << selectedSquare) ||
              selectedSquare == m_SelectedPiece) {
