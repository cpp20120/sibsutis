...
void Application::RenderSettingsPanel(bool* show) {
  ...
  ImGuiColorEditFlags colourEditFlags = ImGuiColorEditFlags_NoInputs |
  ...
  if (ImGui::BeginTable("ColoursTable", 2, ImGuiTableFlags_SizingFixedSame)) {
    ...
    if (ImGui::Combo("##Theme", &currentTheme, themes,
                     sizeof(themes) / sizeof(const char*))) {
      if (currentTheme == 0)
    ...
    ImGui::TableNextColumn();
    ImGui::ColorEdit4("##Dark square colour",
                      glm::value_ptr(m_DarkSquareColour), colourEditFlags);

    ImGui::TableNextRow();
    ImGui::TableNextColumn();
    ImGui::Text("Light square colour");
    ImGui::TableNextColumn();
    ImGui::ColorEdit4("##Light square colour",
                      glm::value_ptr(m_LightSquareColour), colourEditFlags);

    ImGui::TableNextRow();
    ImGui::TableNextColumn();
    ImGui::Text("Legal move colour");
    ImGui::TableNextColumn();
    ImGui::ColorEdit4("##Legal move colour", glm::value_ptr(m_LegalMoveColour),
                      colourEditFlags);
