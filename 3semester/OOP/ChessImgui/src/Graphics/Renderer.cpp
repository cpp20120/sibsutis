...
void Renderer::Init(const glm::mat4& projection, std::string_view vert,
                    std::string_view frag) {
  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

  s_IndexBuffer =
      std::make_unique<IndexBuffer>(maxIndexCount * sizeof(uint32_t));
  s_Shader = std::make_unique<Shader>(vert.data(), frag.data());
  s_VertexBuffer =
      std::make_unique<VertexBuffer>(maxVertexCount * sizeof(Vertex));
  s_VertexArray =
      std::make_unique<VertexArray>(*s_VertexBuffer, s_VertexLayout);
  ...
  for (int32_t i = 0; i < maxTextureCount; i++) textures[i] = i;

  s_Shader->SetUniform("u_Textures", textures,
                       sizeof(textures) / sizeof(uint32_t));
}
...
const glm::mat4& Renderer::GetProjectionMatrix() { return s_ProjectionMatrix; }
...
void Renderer::DrawRect(const glm::vec3& position, const glm::vec2& size,
  ...
  DrawRect(transform, {1.0f, 1.0f, 1.0f, 1.0f}, texture,
           s_TextureCoords.data());
}
...
void Renderer::DrawRect(const glm::vec3& position, const glm::vec2& size,
  ...
  DrawRect(transform, {1.0f, 1.0f, 1.0f, 1.0f}, subTexture->GetTexture(),
           subTexture->GetTextureCoordinates());
}
