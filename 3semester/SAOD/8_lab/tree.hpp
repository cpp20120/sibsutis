...
protected:
    ...
    static void  getArrayElementsFromLeftToRight (TNode* p, std::vector<T>& arr) {
        if (p != nullptr) {
    ...
    static void  getArrayElementsFromUpToDown (TNode* p, std::vector<T>& arr) {
        if (p != nullptr) {
            arr.push_back(p->value);
...
protected:
    ...
    void  remove (TNode* cur, TNode* prev, const T& value) {
        ...
        if (cur->value == value) {
            ...
            if (value < cur->value)  remove (cur->left, cur, value);
            else  remove (cur->right, cur, value);
        }
    }
...
public:
    ...
    void addRecurs(const T& val) {
        this->root =  addRecurs (val, this->root);
    }

    void remove(const T& value) override {
         remove (this->root, nullptr, value);
    }
};
    ...
    void  add (const T& val, Node** cur, Node** prev) {
        for (std::size_t i = 0; i < (*cur)->values.size(); i++) {
            if (val < (*cur)->values.at(i)) {
                ...
                else  add (val, &(*cur)->child_nodes.at(i), cur);
                break;
            }
            else if (i + 1 == (*cur)->values.size()) {
                if ((*cur)->child_nodes.at(i + 1) == nullptr) (*cur)->add(val);
                else  add (val, &(*cur)->child_nodes.at(i + 1), cur);
                break;
            }
        }

        if ((*cur)->values.size() == M) {
            if (prev == nullptr) (*cur) =  newRoot ();
            else  balanced (cur, prev);
        }
    }
...
public:
    ...
    double getMediumHeight() override { return static_cast<double>( getSumLenght (root, 1)) / static_cast<double>(getSize()); }
...
namespace os_tree {
    ...
    protected:
        ...
        void  create(const std::vector<std::pair<T, uint64_t>>& arr, std::size_t left, std::size_t right) {
            if (left < right) {
        ...
        int  sumLengthWaysTreeDOP(OSTree<T>* p, int L) {
            if (p == nullptr) return 0;
            return p->weight * L + sumLengthWaysTreeDOP(p->left, L + 1) + sumLengthWaysTreeDOP(p->right, L + 1);
        }
    ...
    public:
        ...
        double weightedAverageHeightTree() { return static_cast<double>(sumLengthWaysTreeDOP(this->root, 1)) / static_cast<double>(weightTree(this->root)); }
    };
    ...
    protected:
        void  create(const std::vector<std::pair<T, uint64_t>>& arr, int64_t left, int64_t right) {
    ...
    public:
        OSTreeA2(const std::vector<std::pair<T, uint64_t>>& arr) { create(arr, 0, arr.size() - 1); }
    };
}
