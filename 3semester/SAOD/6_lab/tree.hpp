...
class AVLTree : public ITree<T> {
  ...
  /**
   * @brief Recursively removes a value from the AVL tree and rebalances it.
   *
   * This method removes a value from the AVL tree, adjusting balance factors
   * and performing rotations as necessary to maintain the tree's balance.
   *
   ...
   * @return A pointer to the current node, possibly updated after rotation.
   */
  TreeNode<T>* remove(const T& val, TreeNode<T>* p, bool& umen) {
    if (p == nullptr) {
    } else if (val < p->value) {
 ...
 public:
   ...
   *
   * This method provides an interface to remove values from the tree and
