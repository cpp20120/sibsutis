...
struct Student {
  ...
  std::vector<Subject>
      subjects;  ///< Вектор с информацией о предметах студента.
};
 ...
 */
void searchAndUpdateRecord(const std::string& filename,
                           const std::string& searchLastName) {
   ...
   *
   * Ищет студента в векторе по фамилии. Если студент найден, запрашивает у
   * пользователя новые данные (фамилию, имя, отчество и оценки по предметам) и
  ...
  for (auto& student : students) {
    if (student.lastName == searchLastName) {
      ...
      for (int i = 0; i < max_record; ++i) {
        std::cout << "Введите новое название предмета и оценку по предмету."
                  << i + 1 << " (через пробел): ";
   ...
   *
   * Открывает файл для записи и записывает обновленные данные о студентах в
   * него. Каждая строка файла представляет одного студента с его фамилией,
   * именем, отчеством и оценками по предметам.
   *
   * @param filename Имя файла для сохранения обновленных данных.
   * @param students Вектор студентов с обновленными данными.
   */
  std::fstream updatedFile(filename,
                           std::ios::in | std::ios::out | std::ios::trunc);
  for (const auto& student : students) {
    updatedFile << student.lastName << ' ' << student.firstName << ' '
                << student.patronymic << ' ';
    for (const auto& subject : student.subjects) {
...
void deleteRecord(const std::string& filename,
                  const std::string& deleteLastName) {
  std::vector<std::string> records;
  std::ifstream file(filename);
  std::string line;
  while (std::getline(file, line)) {
   ...
   *
   * Использует алгоритм std::remove_if для фильтрации записей с заданной
   * фамилией. Записи с указанной фамилией будут перемещены в конец вектора, а
   * затем удалены.
   *
  ...
  auto it = std::remove_if(records.begin(), records.end(),
                           [deleteLastName](const std::string& record) {
                             std::string lastName =
  ...
  do {
    ...
    switch (choice) {
      ...
      case 2: {
        ...
        std::cin >> newStudent.lastName >> newStudent.firstName >>
            newStudent.patronymic;

        for (int i = 0; i < max_record; ++i) {
          Subject subject;
          std::cout << "Введите название предмета и оценку для предмета "
                    << i + 1 << ": ";
